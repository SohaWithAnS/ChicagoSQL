...
public class Table{
	...
	public static void drop(String table){
		try{
			// delete table record from davisbase_tables and davisbase_columns from /data/catalog dir
			...
			File oldFile = new File(Constants.userDataDir, table+Constants.FILE_TYPE); 
			oldFile.delete();
	...
	public static void createTable(String table, String[] cols){
		try{
			...
			insertInto(Constants.TABLE_CATALOG, values,Constants.catalogDir);
			...
			for(int i = 0; i < newCol.length; i++){
				...
				insertInto("davisbase_columns", value,Constants.catalogDir);
			}
