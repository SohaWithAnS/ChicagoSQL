...
public class DavisBase {
	...
	public static void initialize() 
	{
		...
		try {
			RandomAccessFile catalogTable = new RandomAccessFile(Constants.catalogDir+"/davisbase_tables.tbl", "rw");
			catalogTable.setLength(Constants.PAGE_SIZE);
			catalogTable.seek(0);
			catalogTable.write(0x0D); //records page
			catalogTable.writeByte(0x02); //intnull (size 4, can also use int here)
		...
		try {
			RandomAccessFile catalogColumn = new RandomAccessFile(Constants.catalogDir+"/davisbase_columns.tbl", "rw");
